...
export default function GestureASL() {
  ...
  const GESTURE_TO_PHRASE: Record<string, { text: string; glosses: string[]; entityType: Entity['type'] }> = {
    ...
    Thumb_Up:    { text: 'mild',   glosses: ['mild', 'cough'], entityType: 'severity'  },
    ...
    if (handRes) {
      ...
      if (hand && nose) {
        ...
        if (allergyHoldMsRef.current >= ALLERGY_HOLD_MS && nowAbs - lastEmitAtRef.current >= COOLDOWN_MS) {
          ...
          void emitFromMapping('ALLERGY', nowAbs, typeof score === 'number' ? score : 0.99);
  ...
  useEffect(() => { if (visit.status !== 'recording' && status === 'running') stop(); }, [visit.status]);
  useEffect(() => () => void stop(), []);

  return (
    <article aria-labelledby="asl-heading" className="contrast">
      <header>
        <h3 id="asl-heading">Patient Gestures (MediaPipe)</h3>
        <p className="secondary">
          Status: <strong>{status}</strong> • Last: {lastGesture}
        </p>
      </header>

      <nav aria-label="ASL controls" className="grid">
        <div role="group">
          {status !== 'running' ? (
            <button onClick={start}>Start Gesture Capture</button>
          ) : (
            <button className="secondary" onClick={stop}>Stop Gesture Capture</button>
          )}
        </div>
      ...
      <figure>
        {/* Keep minimal inline styles for the overlay */}
      ...
      {(fupLoading || fup || fupError) && (
        <section aria-live="polite" aria-busy={fupLoading ? true : undefined} className="contrast">
          <h4>Suggested follow-up</h4>
          {fupLoading && <div className="secondary">thinking…</div>}
          {fup && (
            <>
              <p><strong>Q:</strong> {fup.question}</p>
              <p className="secondary">Why: {fup.why}</p>
              <div role="group">
                <button className="secondary" onClick={() => speak(fup.question, { rate: 1 })}>🔊 Ask</button>
                <button className="outline" onClick={() => navigator.clipboard.writeText(fup.question)}>Copy</button>
              </div>
            </>
          )}
          {fupError && <small style={{ color: '#f66' }}>{fupError}</small>}
        </section>
      )}
    </article>
  );
